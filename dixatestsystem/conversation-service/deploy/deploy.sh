...
buildAndDeploy() {
    ...
    docker build -t "${image_name}":latest -t "${image_name}:${git_sha}" -f ../Dockerfile --build-arg NPM_TOKEN="${npm_token}" --no-cache ../.
...
kubectl patch deployment conversation-service -n production -p "{\"spec\": {\"template\": {\"metadata\": { \"labels\": {  \"redeploy\": \"$(date +%s)\"}}}}}"
